...
# https://github.com/linto-project/linto-sp2/blob/2c8475fb7b0f98e2cd46d42137e30394f4c51bed/audio/extract_feat/speechTools/WADA.py
...
def read_audio_ffmpeg_from_start_to_duration(path, start, duration):
    ...
    :return: le waveform (np) de l'audio en 16bit 16kHz mono et amplitude de (-1 à 1)
    """
    command = ['ffmpeg',
               '-i', path,
               '-f', 's16le',
...
import ffmpeg
SAMPLE_RATE = 16000
def load_audio(file: str, start, duration, sr: int = SAMPLE_RATE):
    """
    ...
    Returns
    -------
    A np array containing the audio waveform, in float32 dtype.
    """
    try:
        # This launches a subprocess to decode audio while down-mixing and resampling as necessary.
        # Requires the ffmpeg CLI and `ffmpeg-python` package to be installed.
        out, _ = (
            ffmpeg.input(file, threads=0, ss=start, t=duration)
...
def compute_snr(path, start, duration):
    ...
    if (isinstance(duration, float) or isinstance(duration, int)) and \
        ...
        try:
            audio = load_audio(path, start, duration)
    ...
    dSNRix = np.max( [ i for i in range(0, len(table)) if table[i][1] < dVal3])
    ...
    snr = 10 * np.log10(np.maximum(np.maximum(dSigEng, min_val) / np.maximum(dNoiseEng, min_val),
                                       min_val))
...
def compute_wada_snr(path, time_SNR_window=30, min_time_SNR_window=3):
    ...
    if duration > time_SNR_window:
        start_list = np.arange(0, duration, time_SNR_window)
...
def compute_wada_snr_seg(path, time_SNR_window=3, last_window=False):
    ...
    if duration > time_SNR_window:
        ...
        snrs.append(compute_snr(path, start=0, duration=duration))
    return snrs


def wada(audiofile=None):
    """"
    ...
    if not audiofile:
        parser = argparse.ArgumentParser(description="SNR estimator WADA")
        parser.add_argument('--audio', '-i', dest="wavfile", required=True, help="wav file")
        args =parser.parse_args()
        audiofile = args.wavfile
    logging.basicConfig(stream=sys.stdout, level=logging.DEBUG)
    snr = compute_wada_snr_seg(audiofile)
    # print('{0} {1}'.format(audiofile, snr))
    return snr
